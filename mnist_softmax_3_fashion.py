...
def main(_):
    ...
    while (stored_correct < num_each_to_store or stored_incorrect < num_each_to_store) and idx < len(mnist.test.images):
        ...
        if correct and stored_correct < num_each_to_store:
            stored_correct += 1
            plt.savefig("output_images/fashion_success_{}.png".format(real_label.astype(str)))
        elif not correct and stored_incorrect < num_each_to_store:
            stored_incorrect += 1
            plt.savefig("output_images/fashion_fail_{}_{}.png".format(real_label.astype(str), pred.astype(str)))
        idx += 1
